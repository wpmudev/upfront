...
class Upfront_Output {
	...
	public function apply_layout () {
		...
		// Also check for inline slider
		if ( strpos($html, 'upfront-inline_post-slider') !== false )
			$has_slider = true;

		// Add default scripts/styles that can be overriden as needed
		if ( $has_map || is_user_logged_in() )
			upfront_add_element_script('upfront-default-map', array('scripts/default-map.js', dirname(__FILE__)));
		if ( $has_slider || is_user_logged_in() ){
