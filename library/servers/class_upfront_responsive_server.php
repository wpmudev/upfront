...
class Upfront_Server_ResponsiveServer extends Upfront_Server {
	...
	public function update_breakpoints() {
		...
		$responsive_settings = Upfront_Cache_Utils::get_option_replaced('upfront_' . get_stylesheet() . '_responsive_settings');
		...
		if (!has_action('upfront_update_responsive_settings')) {
			Upfront_Cache_Utils::update_option_replaced('upfront_' . get_stylesheet() . '_responsive_settings', json_encode($responsive_settings));
		}
