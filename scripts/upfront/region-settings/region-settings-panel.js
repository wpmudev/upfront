(function ($) {
define([
	"text!upfront/region-settings/templates/region_edit_panel.html"
], function (regionEditPanelTpl) {
	var l10n = Upfront.Settings && Upfront.Settings.l10n ?
		Upfront.Settings.l10n.global.views : Upfront.mainData.l10n.global.views;

	var RegionSettingsPanel = Backbone.View.extend({
		...
		events: {
			...
			'click .uf-settings-panel__title': 'toggleAdvancedSettings',
			'click .command-exit': 'exit'
		},

		open: function () {
			this.trigger('region:settings:open');
			// do something to open this
			this.render();
			this.delegateEvents();

			return { always: function(whatToDo) {
					// Dummy function
		...
		close: function () {
			console.log('closing');
			this.trigger('region:settings:close');
			this.$el.remove();
			this.stopListening();
			$('#region-settings-sidebar').width(0).css('opacity', 0);
			// do something to close this
		},

		render: function() {
			this.$el.removeClass('image-region-settings');
			if (this.model.get_breakpoint_property_value('background_type') === 'image') {
				this.$el.addClass('image-region-settings');
			}

			this.render_modal(this.$el);
			this.$el.append('<div class="close_button">' + l10n.ok_camel_case + '</div>');
			$('#region-settings-sidebar').removeClass();
			$('#region-settings-sidebar').addClass('region-settings-sidebar-' + this.model.get_breakpoint_property_value('background_type'));
		},
		...
		render_modal: function ($content, $modal) {
			...
			var	bg_type = new Upfront.Views.Editor.Field.Select({
					model: this.model,
					...
					change: function () {
						...
						me.$el.removeClass('image-region-settings');
						if (value === 'image') {
							me.$el.addClass('image-region-settings');
						}
						$('#region-settings-sidebar').removeClass();
						$('#region-settings-sidebar').addClass('region-settings-sidebar-' + this.model.get_breakpoint_property_value('background_type'));
					}
			...
			if ( !is_responsive && is_region ) {
				var region_name = new Upfront.Views.Editor.Field.Text({
						model: this.model,
						name: 'title',
					...
					make_global = new Upfront.Views.Editor.Field.Button({
						model: this.model,
						name: 'scope',
						label: l10n.make_global,
						classname: 'upfront-region-bg-setting-localize upfront-region-bg-setting-make_global',
						compact: true,
						on_click: function () {
					...
					localize_region = new Upfront.Views.Editor.Field.Button({
						model: this.model,
						name: 'localize',
					...
					name_save = new Upfront.Views.Editor.Field.Button({
						model: this.model,
						name: 'save',
						label: l10n.edit_region_header_ok,
						compact: true,
						...
						on_click: function () {
							...
							me.removeNoEditOverlays();
						}
					})
					;
			}
			if ( is_layout ){
				var contained_region = new Upfront.Views.Editor.Field.Number({
					model: this.model,
			...
			if ( is_region && this.model.is_main() ){
				...
				region_global = new Upfront.Views.Editor.Field.Checkboxes({
					model: this.model,
					name: 'scope',
					multiple: false,
					values: [
					...
					region_type = new Upfront.Views.Editor.Field.Select({
						model: this.model,
						name: 'type',
						label: 'Region Type',
						default_value: 'wide',
					...
					region_nav = new Upfront.Views.Editor.Field.Checkboxes({
						model: this.model,
					...
					region_behavior = new Upfront.Views.Editor.Field.Radios({
						model: this.model,
						name: 'behavior',
				...
				var region_restrict = new Upfront.Views.Editor.Field.Checkboxes({
					model: this.model,
			...
			if ( is_region ) {
				...
				// Padding Settings
				var lock_padding = new Upfront.Views.Editor.Field.Checkboxes({
					model: this.model,
					...
					change: function(value) {
						me.model.set_breakpoint_property('bg_padding_type', this.get_value());
					}
				});
				var top_bg_padding_num = new Upfront.Views.Editor.Field.Number({
						model: this.model,
						use_breakpoint_property: true,
					...
					bottom_bg_padding_num = new Upfront.Views.Editor.Field.Number({
						model: this.model,
						use_breakpoint_property: true,
					...
					bg_padding_num = new Upfront.Views.Editor.Field.Number({
						model: this.model,
						use_breakpoint_property: true,
			...
			//TODO $modal.addClass('upfront-region-modal-bg');
			...
			if ( !is_responsive && is_region ) {
				...
				$region_name.on('click', '.upfront-region-name-edit-trigger', function(e){
					e.preventDefault();
					$region_name.addClass('in-editing');
			...
			if ( !is_responsive && is_region && this.model.is_main() ) {
				region_type.render();
				$region_type.prepend(region_type.$el);
				region_nav.render();
			...
			if ( !is_responsive && is_region && ( this.model.is_main() || sub == 'top' || sub == 'bottom' ) ) {
				...
				if ( ( !has_sticky && this.for_view.$el.height() <= 300 ) || this.model.get('sticky') ) {
					var region_sticky = new Upfront.Views.Editor.Field.Checkboxes({
						model: this.model,
						name: 'sticky',
						default_value: '',
						layout: 'horizontal-inline',
			...
			if ( is_region ){
				// Padding Settings
				lock_padding.render();
				$region_padding_type.append(lock_padding.$el);
		...
		_render_tab_template: function($target, primary, secondary, template){
			var $template = $(regionEditPanelTpl),
				$tab = false, tpl = false
				;
			if (template) {
			...
			if ( secondary ) {
				...
				$tab.find('.upfront-region-bg-setting-tab-secondary').hide();
			}
			$target.html('');
			$target.append($tab);
		},
		...
		render_modal_tab_color: function ($tab) {
			...
			this._color_item.$el.find('label').remove();
			this._render_tab_template($tab, this._color_item.$el, '');
			if ($tab.find('.sp-light-label-color').size() < 1)
				$tab.find('.sp-replacer').after('<label class="sp-light-label-color">BG Color</label>');
		},
		// Image tab
		...
		addNoEditOverlays: function() {
			$('.upfront-region-bg-setting-type, .upfront-region-bg-setting-region-type, .upfront-region-bg-setting-tab-color, .upfront-region-bg-setting-tab-image, .upfront-region-bg-setting-tab-video, .upfront-region-bg-setting-tab-slider, .upfront-region-bg-setting-tab-color, .upfront-region-bg-setting-padding, .upfront-region-bg-setting-footer', '#region-settings-sidebar').append('<div class="region-no-edit-overlay" />');
		},
		removeNoEditOverlays: function() {
			$('#region-settings-sidebar').find('.region-no-edit-overlay').remove();
		},
