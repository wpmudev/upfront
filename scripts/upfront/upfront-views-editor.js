...
*/
	...
	var Loading = Backbone.View.extend({
		...
		render: function () {
			...
			if (this.options.loading) this.update_loading_text(this.options.loading);
			if (this.options.loading_notice) this.update_loading_notice(this.options.loading_notice);
		...
		update_loading_text: function (loading) {
			var $node = this.$el.find('.upfront-loading-text');
			if ( $node.length > 0 ) $node.text(loading);
			else this.$el.append('<p class="upfront-loading-text">' + loading + '</p>')
		},
		update_loading_notice: function (notice) {
			var $node = this.$el.find('.upfront-loading-notice');
			if ( $node.length > 0 ) $node.text(notice);
			else this.$el.append('<p class="upfront-loading-notice">' + notice + '</p>');
		},
