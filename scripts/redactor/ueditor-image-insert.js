...
var ImageInsert = base.ImageInsertBase.extend({
		...
		this.controlsData = [
			{
				...
				type: 'dialog',
				icon: 'link',
				tooltip: Upfront.mainData.l10n.global.ueditor.link_image,
			...
			},
			{
				...
				type: 'simple',
				icon: 'caption',
				tooltip: Upfront.mainData.l10n.global.ueditor.toggle_caption,
				active: _.bind(this.get_caption_state, this)
			}
		];

		if (this.allow_alignment(group_width_cls)) {
			this.controlsData.unshift({
				id: 'style',
				type: 'dialog',
				icon: 'style',
				tooltip: Upfront.mainData.l10n.global.ueditor.alignment,
	...
	render: function () {
		...
		this.$(".ueditor-insert-variant-group")
			.append('<a href="#" contenteditable="false" class="upfront-icon-button upfront-icon-button-delete ueditor-insert-remove"></a>');
	...
	importFromImage: function (image) {
		...
		if (link.origin != window.location.origin) imageData.isLocal = 0;
		...
		if (!attachmentId) {
			...
			if (attachmentId) imageData.attachmentId = attachmentId[1];
			else imageData.attachmentId = false;
		}
		...
		if ($group.length) {
			...
			if (imageData.variant_id) {
				imageData.alignment = utils.BasicImageVariants.findWhere({
					"vid": $group.data("variant")
				});
			}
		} else {
		...
		insert.render();
		var replacee = image.hasClass(".upfront-inserted_image-basic-wrapper")
			? image
			: image.closest(".upfront-inserted_image-basic-wrapper")
		;
