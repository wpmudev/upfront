...
var PostImageInsert = base.ImageInsertBase.extend({
	...
	shortcode_tpl: _.template(
		$(tpls).find('#post-image-insert-shortcode-tpl').html().replace(/\s+/g," ")
	),
		...
		this.controlsData = [
			{
				...
				icon: 'style',
				tooltip: l10n.style,
				view: this.getStyleView()
			},
			{
				id: 'change_image',
				type: 'simple',
				icon: 'change_image',
				tooltip: l10n.change_image
			},
			{
				id: 'link',
				type: 'dialog',
				icon: 'link',
				tooltip: l10n.link_image,
				view: this.getLinkView()
			},
			{
				id: 'toggle_caption',
				type: 'simple',
				icon: 'caption',
				tooltip: l10n.toggle_caption,
				active: _.bind(this.get_caption_state, this)
			}
		];
		this.createControls();

		if (opts.start) return this.start(opts.start);
	},
	...
	prepare_data: function () {
		...
		data.style.label_id = data.style.label && data.style.label.trim() !== ""
			? "ueditor-image-style-" +  data.style.label.toLowerCase().trim().replace(" ", "-")
			: data.style.vid
		;
		data.image = this.get_proper_image();

		if (data.linkType == "show_larger_image") data.linkUrl = data.image.src;
		...
		var $group = this.$el.find(".ueditor-insert-variant-group"),
			...
			padding_left = parseFloat(
				$(".upfront-content-marker-contents>*").css("padding-left")
			) / ge.col_size ,
			padding_right = parseFloat(
				$(".upfront-content-marker-contents>*").css("padding-right")
			) / ge.col_size,
		...
		if (this.$editor.hasClass('upfront-indented_content')) {
			...
			this.$el.attr(
					'style',
					'margin-left: ' +
					((padding_left * col_size * -1) - object_padding_left) + 'px; ' +
					'margin-right: ' +
					(( padding_right * col_size * -1 ) - object_padding_right) + 'px;'
			);
		}
		...
		if (style_variant && style_variant.group && style_variant.group.float) {
			...
			if (style_variant.group.float == 'left') {
				data.style.group.marginLeft = (
					padding_left - Math.abs(style_variant.group.margin_left)
				) * col_size;
				data.style.group.marginRight = 0;
			} else if (style_variant.group.float == 'right') {
				data.style.group.marginRight = (
					padding_right - Math.abs(style_variant.group.margin_right)
				) * col_size;
				data.style.group.marginLeft = 0;
			} else if (style_variant.group.float == 'none') {
				data.style.group.marginLeft = (
					padding_left - Math.abs(style_variant.group.margin_left) +
					Math.abs(style_variant.group.left)
				) * col_size;
				data.style.group.marginRight = 0;
...
var WP_PostImageInsert = base.ImageInsertBase.extend({
	...
	tpl : _.template(
		$(tpls).find('#post-image-insert-wp-tpl').html()
	),
	shortcode_tpl : _.template(
		$(tpls).find('#post-image-insert-shortcode-wp-tpl').html()
	),
		...
		this.controlsData = [
			{
				id: 'wp_style',
				type: 'dialog',
				icon: _.bind(this.get_style_icon, this),
				tooltip: l10n.style,
				view: this.getStyleView(),
				hideOkButton: true
			},
	...
	render: function () {
		...
		if (
			typeof caption_ueditor !== undefined &&
			caption_ueditor &&
			typeof caption_ueditor.redactor !== undefined
		) {
	...
	getStyleView: function () {
		if (this.styleView) return this.styleView;
	...
	controlEvents: function () {
		...
		this.listenTo(this.controls, 'control:click:toggle_caption', function (control) {
			var new_state = 1,
				style = Upfront.Util.clone(this.data.toJSON().style)
			; // cloning to make it trigger the BB model change event

			if (!this.get_caption_state()) new_state = 0;
